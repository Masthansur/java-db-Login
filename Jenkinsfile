pipeline {
    ...
    stages {
        stage('git fetch') {
            steps {
                git branch: "${params.branch}", credentialsId: 'ansadmin-key', url: 'https://github.com/Masthansur/java-db-Login.git'
            }
        }
        ...
        when{
            ...
            steps {
                ...
                deploy adapters: [tomcat8(credentialsId: 'tomcat-cred', path: '', url: 'http://172.31.86.32:8080')], contextPath: 'login-release-branch', onFailure: false, war: '**/*.war'
            }
        }
